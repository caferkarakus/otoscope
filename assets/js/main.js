  ...
  container.innerHTML = `
    <div class="staging-ref-layout">
      <nav class="staging-ref-sidebar">
        ${items.map((item, i) => `<button type="button" class="staging-ref-nav-btn" data-index="${i}">${getTitle(item)}</button>`).join("")}
      </nav>
      <div class="staging-ref-main"></div>
    </div>
  `;

  const nav = container.querySelector(".staging-ref-sidebar");
  const main = container.querySelector(".staging-ref-main");

  function select(index) {
    nav.querySelectorAll(".staging-ref-nav-btn").forEach((b, i) => b.classList.toggle("active", i === index));
...
function wireQuestionCard(q, main) {
  const buttons = main.querySelectorAll(".option-btn");
  let answered = false;
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      main.querySelector(".explanation").classList.add("visible");
    });
  });
}
...
const CONTENT_RENDERERS = {
  ozetler(container) {
    renderSidebarSection(container, {
      items: filterByCategory(SUMMARY_CARDS),
      ...
      `,
      emptyText: "Bu yan dal için henüz özet kart eklenmedi."
    });
  },

  anatomi(container) {
    renderSidebarSection(container, {
      items: filterByCategory(ANATOMY),
      ...
      `,
      emptyText: "Bu yan dal için henüz anatomi içeriği eklenmedi."
    });
  },
  ...
  aciller(container) {
    renderSidebarSection(container, {
      items: filterByCategory(EMERGENCIES),
      getTitle: (e) => e.title,
      getBody: (e) => `
        <h2>${e.title}</h2>
        <div class="modal-section">
      ...
      `,
      emptyText: "Bu yan dal için henüz acil durum eklenmedi."
    });
  },
...
const SEARCH_TYPE_SOURCES = {
  ozetler: SUMMARY_CARDS,
  anatomi: ANATOMY,
  sorular: QUESTIONS,
  tus: TUS_QUESTIONS,
  vakalar: CASES,
...
function goToSearchResult(entry) {
  ...
  const btn = container.querySelector(`.staging-ref-nav-btn[data-index="${index}"]`);
  if (btn) btn.click();
}
